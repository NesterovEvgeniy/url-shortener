...
async def search_links(original_url: str, db: Session = Depends(get_db)):
    ...
    """
    links = db.query(Link).filter(Link.original_url.contains(original_url)).all()
    return links
...
# Перенаправление и проверка срока истечения
@router.get("/{short_code}/redirect/", summary="Перенаправление на оригинальный URL", description="Перенаправление на оригинальный URL и запись статистики посещений")
async def redirect_to_original(short_code: str, request: Request, db: Session = Depends(get_db)):
    """Перенаправление на оригинальный URL
